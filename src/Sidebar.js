...
class Sidebar extends React.Component {
  ...
  setCourses(){
    this.props.setCourses(this.searchAndFilter.searchAndFilter(this.props.courses, this.search.current.value, this.subject.current.value, this.minimumCredits.current.value, this.maximumCredits.current.value, this.keyword.current.value));
  }

  getKeyword = (keywordRef) =>{
    this.keyword = keywordRef;
    this.setCourses();
  }
  ...
  render() {
    return (
      <>
        <Card style={{width: 'calc(20vw - 9px)', marginTop: '5px', marginLeft: '7px', height: '340px', position: 'fixed', backgroundColor: "#d6d6d6"}}>
          <Card.Body style={{marginTop: '5px'}}>
            ...
            <Form>
              ...
              <InterestArea keywords={this.props.keywords} keyword={this.keyword} getKeyword={this.getKeyword}/>
